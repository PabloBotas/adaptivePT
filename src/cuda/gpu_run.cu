                   ...
                   float4* scorer)
{
    ...
    calculateRays_kernel<<<nblocks, NTHREAD_PER_BLOCK_RAYS>>>(total_spots, scorer, spots_per_beam_gpu);
                            ...
                            const float3& ct_offsets)
{
    ...
    gpuErrchk( cudaMemcpy(angles_gpu, temp, sizeof(float2)*angles.size(), cudaMemcpyHostToDevice) );
