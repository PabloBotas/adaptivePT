...
def analize_vf(vf_file):
    ...
    ax.quiver(z, y, vz, vy, d, angles='xy', cmap=plt.cm.YlOrRd, pivot='tail', alpha=0.75)
...
def analize_tramp(shifts_file, tramp_files):
    ...
    for tramp_num, tramp_file in enumerate(tramp_files, start=0):
        ...
        ax.plot(tramp_e+de, 'ro', markersize=2)
        ...
        nbins = 25
        ax.hist(de, nbins)
        ax.set_xlabel('Shift size (MeV)', fontsize=8)
        ax.tick_params(labelsize=6)
        ...
        ax.plot(tramp_y + y, 'ro', markersize=2)
        ax.set_xlabel('Spot number', fontsize=8)
        ...
        scat_colors = np.array(plt.cm.hsv(layer_id / layer_id.max()))
        ax.scatter(tramp_x, tramp_y, s=10, linewidth=0.5, zorder=1,
                   edgecolors=scat_colors-np.array([0, 0, 0, 0.75]), facecolors='')
        ax.scatter(x, y, s=10, linewidth=0.5, zorder=2, color=scat_colors-np.array([0, 0, 0, 0]))
        ax.tick_params(labelsize=6)
        ...
        nbins = 25
        ax.hist(d, range=[0.95*d.min(), 1.05*d.max()], bins=nbins)
        ax.set_xlabel('Shift size (mm)', fontsize=8)
        ax.tick_params(labelsize=8)
        ...
        #     bool_mask = tramp_e == layer_energy
        #     pos_x = tramp_x[bool_mask]
        #     pos_y = tramp_y[bool_mask]
        #     shift_x = x[bool_mask]
        #     shift_y = y[bool_mask]
